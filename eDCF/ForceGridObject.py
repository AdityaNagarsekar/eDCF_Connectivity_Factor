    ...
    def grid_points_finder(self, num_workers: int, grid_bounds_full: np.ndarray, info: np.ndarray, spacing_points: float, direct_conversion_ctrl: bool = False):
        ...
        if not direct_conversion_ctrl:
            ...
            if len(self.__data_set) > num_workers:
                dim = len(self.__data_set[0])
                third_axis = num_workers
                while third_axis < len(self.__data_set):
                    if len(self.__data_set) % (dim * third_axis) == 0:
                        break
                    else:
                        third_axis += 1

                data_set_list = list(self.__data_set.reshape(third_axis, -1, dim))
            else:
                data_set_list = list(self.__data_set)

            grid_bounds_full_list = []
            info_list = []
            spacing_points_list = []

            for i in range(len(data_set_list)):
                grid_bounds_full_list.append(grid_bounds_full)
                info_list.append(info)
                spacing_points_list.append(spacing_points)

            params = list(zip(data_set_list, grid_bounds_full_list, info_list, spacing_points_list))

            # FIXME: multiprocessing not working everytime(crashes sometimes)
            
            # with multiprocessing.Pool(processes=num_workers) as pool:

            #     grid_set_list = pool.starmap(self.direct_gridder, params)

            # final_set: set = set()

            # for grid_set in grid_set_list:

            #     final_set.update(grid_set)

            data_set = np.vstack(data_set_list)

            final_set = self.direct_gridder(data_set, grid_bounds_full, info, spacing_points)
