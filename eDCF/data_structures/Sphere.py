...
class Sphere:
    ...
    def __init__(self, identity: int, radius: float = 0.0, center: Tuple[float, float, float] = (0.0, 0.0, 0.0), noise_rate: float = 1.0):
        """
    ...
    def generate(self, n: int = 100) -> np.ndarray:
        ...
        self.__datapoints = np.zeros((n, 4), float)  # initializing datapoints
        ...
        for t, p in zip(theta, phi):  # assigning loop
            ...
            if t == 90 or t == 270:
                ...
                x = self.__radius * math.sin(math.radians(p)) * math.cos(math.radians(t)) + noise_x

            if p == 90 or p == 270:
                # If phi is 90 or 270, z = 0, handle precision manually
                z = 0 + noise_z
            else:
                z = self.__radius * math.cos(math.radians(p)) + noise_z
            ...
            self.__datapoints[i, 3] = self.__identity  # identity/label assignment
