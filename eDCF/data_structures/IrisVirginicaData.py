...
class IrisVirginicaData:
    """
    Encapsulates data from the Iris Virginica class with functionalities to perform PCA for dimensionality reduction.
    This class allows extraction of Iris Virginica data after applying PCA to the whole Iris dataset.
    """

    def __init__(self, identity: int = 3, pca_components: int = 2):
        """
        Constructor for IrisVirginicaData.

        :param identity: The class identity (label) for Iris Virginica. Default is 2 as per sklearn's labeling.
        :param pca_components: Number of principal components for PCA. Default is 2.
        """
        self.__identity = identity  # Class identity (label) for Iris Virginica
    ...
    def get_identity(self) -> int:
        """Return the identity (class label) of the Iris Virginica class."""
    ...
    def get_virginica_data(self, transformed: bool = True) -> np.ndarray:
        """
        Extract and return data points belonging to the Iris Virginica class (label 2).
        ...
        :return: A numpy array of Iris Virginica data points
        """
        virginica_mask = self.__labels == (self.__identity - 1)  # Mask for Iris Virginica (label 2)

        if transformed:
            return self.__datapoints_pca[virginica_mask]
        else:
            return self.__datapoints_original[virginica_mask]
    ...
    def generate(self, n: int = 100):
        ...
        data = self.get_virginica_data(transformed=True)
